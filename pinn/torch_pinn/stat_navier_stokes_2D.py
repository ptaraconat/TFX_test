...
def get_pde_loss(x,y,model):
    ...
    u = res[:,0].unsqueeze(dim = 1)
    v = res[:,1].unsqueeze(dim = 1)
    p = res[:,2].unsqueeze(dim = 1)
    ...
    loss_cont = nn.MSELoss()(residual_cont,torch.zeros_like(residual_cont))
...
def get_velocity_data_loss(x,y,u,v,model):
    ...
    u_pred = predictions[:,0].unsqueeze(dim = 1)
    v_pred = predictions[:,1].unsqueeze(dim = 1)
    del predictions
    # Calculate Loss 
...
def get_pressure_data_loss(x,y,p,model):
    ...
    input_data = torch.cat((x,y), dim = 1)
    predictions = model(input_data)
    p_pred = predictions[:,2].unsqueeze(dim = 1)
    del predictions
    # Calculate Loss 
...
class PINN_Trainer:
    ...
    def __init__(self, space_domainx = 2, space_domainy = 2, n_col_points = 300, n_bc_points = 100):
        ...
        x_vel_data = self.x_vel_data.numpy()
        y_vel_data = self.y_vel_data.numpy()
        x_pres_data = self.x_pres_data.numpy()
        y_pres_data = self.y_pres_data.numpy()
...
if __name__ == '__main__':
    ...
    with torch.no_grad() : 
        ...
        res = model(input_data)
        solution = res[:,0].reshape(X.shape, Y.shape)
    ...
    with torch.no_grad() : 
        input_data = torch.stack([X.flatten(),Y.flatten()], dim = 1)
        res = model(input_data)
        solution = res[:,1].reshape(X.shape, Y.shape)
        plt.figure(figsize = (8,6))
        sns.heatmap(solution, cmap = 'jet')
    ...
    with torch.no_grad() : 
        input_data = torch.stack([X.flatten(),Y.flatten()], dim = 1)
        res = model(input_data)
        solution = res[:,2].reshape(X.shape, Y.shape)
        plt.figure(figsize = (8,6))
        sns.heatmap(solution, cmap = 'jet')
